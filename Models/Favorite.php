...
class favorite
{
  ...
  public function favoriteDone($company_id,$intern_id)
  {
    ...
    $stmt->bindValue(':company_id', $company_id, PDO::PARAM_INT);
    $stmt->bindValue(':post_id', $intern_id, PDO::PARAM_INT);
  ...
  {
    $sql = 'DELETE FROM favorite WHERE company_id = :company_id AND post_id = :post_id';
    ...
    $stmt->bindValue(':post_id', (int)$intern_id, PDO::PARAM_INT);
    $favorite = $stmt->execute();
    $pdo->commit();
    return $favorite;
    } catch(\Exception $e) {
